...
export function CartItem({ item }: CartItemProps) {
  ...
  const handleUpdate = async (newQty: number) => {
    ...
    try {
      await updateItem({ productId: item.product._id, quantity: newQty })
    } catch {
      ...
      <div className="flex flex-1 flex-col justify-between">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1 rounded-lg border border-border">
            <Button variant="ghost" size="icon" className="h-7 w-7 rounded-l-lg rounded-r-none" onClick={() => handleUpdate(item.quantity - 1)}>
              <Minus className="h-3 w-3" />
            </Button>
            <span className="w-8 text-center text-sm font-medium">{item.quantity}</span>
            <Button variant="ghost" size="icon" className="h-7 w-7 rounded-l-none rounded-r-lg" onClick={() => handleUpdate(item.quantity + 1)}>
